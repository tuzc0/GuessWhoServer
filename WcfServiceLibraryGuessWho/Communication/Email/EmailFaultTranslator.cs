...
namespace GuessWhoServices.Communication.Email
{
    internal static class EmailFaultTranslator
    {
        internal static FaultException<ServiceFault> ToInfrastructureEmailFault(EmailSendResult result)
        {
            if (result == null)
            {
                return FaultsFactory.Create(InfrastructureEmailFaultKeys.CODE_EMAIL_UNEXPECTED_ERROR);
            }
            ...
            switch (errorCode)
            {
                case EmailInternalCodes.RECIPIENT_INVALID:
                    return FaultsFactory.Create(InfrastructureEmailFaultKeys.CODE_EMAIL_RECIPIENT_INVALID);

                case EmailInternalCodes.AUTH_ERROR:
                    return FaultsFactory.Create(InfrastructureEmailFaultKeys.CODE_EMAIL_AUTHENTICATION_FAILED);

                case EmailInternalCodes.CONFIG_MISSING:
                    return FaultsFactory.Create(InfrastructureEmailFaultKeys.CODE_EMAIL_CONFIGURATION_MISSING);

                case EmailInternalCodes.TIMEOUT:
                    return FaultsFactory.Create(InfrastructureEmailFaultKeys.CODE_EMAIL_TIMEOUT);


                case EmailInternalCodes.CONFIG_ERROR:
                    return FaultsFactory.Create(InfrastructureEmailFaultKeys.CODE_EMAIL_CONFIGURATION_ERROR);

                case EmailInternalCodes.UNAVAILABLE:
                    return FaultsFactory.Create(InfrastructureEmailFaultKeys.CODE_EMAIL_UNAVAILABLE);
        ...
        private static FaultException<ServiceFault> MapUnknownByStatus(EmailSendResult result)
        {
            ...
            {
                return FaultsFactory.Create(InfrastructureEmailFaultKeys.CODE_EMAIL_SEND_FAILED);
            }
